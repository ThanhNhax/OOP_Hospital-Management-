...
public class HospitalHandler {
    ...
    private DoctorManager docMan = new DoctorManager(deptMan);
    ...
    public void handleAddNew() {
        while (true) {
            ...
            if (Choose.equals("0"))
                ...
                String deptID = Validator.readPattenString("Nhap Department ID: ", id -> !deptMan.isDuplicateID(id),
                        "Loi: DepartmentID da ton tai!!");
                // Tiếp tục nhập tên dept
                String deptName = Validator.readNonEmptyString("Nhap ten Khoa: ");
                // Ngày tạo là lúc nhập -> chỉ cần gọi hàm Constructor điền thông tin vào với
                // new Date(), ngày lastUpdate = null vì chưa có Update.
                ...
                String docID = Validator.readPattenString("Nhap Doctor ID: ", id -> !docMan.isDuplicateID(id),
                        "Loi: Doctor ID da ton tai!!");
                ...
                // 3. Nhap deptID - bắt buộc đã tồn tại, nếu nhập sai bắt buộc nhập lại deptID
                // đến khi đúng, thay vì bắt nhập lại toàn bộ thông tin
                String deptID = Validator.readPattenString("Nhap Department ID: ", id -> deptMan.findByID(id) != null,
                        "Loi: Department ID khong ton tai!!"); // Nếu ID không tồn tại thì trả về null -> bắt buộc phải
                                                               // != null
            ...
            // Nếu không chọn 0-2 thì báo lỗi
                ...
                Validator.Notice("Loi: Vui long chi nhap tu 0-2!!");
        }
    }
    ...
    public void handleDelete() {
        while (true) {
            ...
            // 1. Xóa dept -> điều kiện ràng buộc: không được xóa nếu còn doc
                ...
                String deptID = Validator.readPattenString("Nhap Department ID can xoa: ",
                        id -> deptMan.findByID(id) != null, "Loi: Department ID khong ton tai!!");
                ...
                if (!deptMan.hasDoctor(deptID, docMan)) {
                    ...
                    if (confirm) {
                        ...
                        Validator.Notice("Huy xoa khoa!!");
                }
                // Ngược lại thì báo lỗi
                else
                    Validator.Notice("Loi: Van con ton tai bac si trong khoa!! Khong the xoa!!");
            }
            // 2. Xóa doc -> xác nhận confirm trước khi xóa
            else if (Choose.equals("2")) {
                // Tìm docID để xóa
                String docID = Validator.readPattenString("Nhap Doctor ID can xoa: ", id -> docMan.findByID(id) != null,
                        "Loi: Doctor ID khong ton tai!!");
                ...
                if (confirm) {
                    ...
                    Validator.Notice("Huy xoa bac si!!");
            } else
                Validator.Notice("Loi: Vui long chi nhap tu 0-2!!");
        }
    }
    ...
    private void handleUpdateDept() {
        ...
        if (deptMan.update(updDept))
            Validator.Notice("Cap nhat Khoa thanh cong!!");
        deptMan.saveToFile(deptFile);
    }
    ...
    private void handleUpdateDoc() {
        ...
        // 4. DeptID - nếu nhập mới thì bắt buộc có trong deptList, nếu không thì bắt
        // nhập lại đến khi đúng.
        ...
        if (docMan.update(updDoc))
            Validator.Notice("Cap nhat bac si thanh cong!!");
        docMan.saveToFile(docFile);
    }
