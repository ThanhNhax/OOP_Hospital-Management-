package hospital;

public class DeparmtnetManagement {
    public
    // In toàn bộ danh sách khoa.
    void showDepartmentList() {

    }

    // Thêm khoa mới (check departmentID không được trùng).
    void addDepartment() {

    }

    // Sửa thông tin khoa theo departmentID (khi sửa xong gán lastUpdateDate là ngày
    // hiện tại; nếu nhập khoảng trắng thì giữ nguyên thông tin cũ).
    void updateDepartment() {

    }

    // Xóa khoa (không được xóa nếu đang có Bác sĩ thuộc khoa đó)
    void deleteDepartment() {

    }

    // Tìm khoa theo departmentID
    void searchDepartmentByID() {

    }

    // Ghi/Đọc file department.dat
    void saveToFile() {
    }

    void loadFromFile() {
    }
}
