...
public class HospitalHandler {
    ...
    public void handleDelete(){
        while (true){
            ...
            else if (Choose.equals("1")) deptHan.deleteDepartment(docHan.getDocMan());
            // 2. Xóa doc -> xác nhận confirm trước khi xóa
            else if (Choose.equals("2")) docHan.deleteDoctor(patHan.getPatMan());
            else if (Choose.equals("3")) patHan.deletePatient();
            else OutputViewer.errChoice(0, 3);
        }
    }
    ...
    // 5. Display a patient: Khi tìm 1 đối tượng bệnh nhân -> dùng while cho tìm liên tục đến khi chọn thoát
    ...
    public void handleDisplayAll(){
        while (true){
            ...
            // Nếu chọn 0 thì return để thoát luôn
            if (Choose.equals("0")) return;
            else if (Choose.equals("1")) deptHan.getDeptMan().showAll();
            else if (Choose.equals("2")) docHan.getDocMan().showAll();
            else if (Choose.equals("3")) patHan.getPatMan().showAll();
            else OutputViewer.errChoice(0, 3);
        }
    }
