...
public class PatientHandle {
    ...
    public void addPatient(){
        String patID = InputValidator.readValidString("Patient ID (Enter 0 to cancel)",null, "add", id -> !id.isEmpty() && patMan.findByID(id) == null, "Error: Patient ID must not be empty and must not exist!!");
        if (patID == null){
    ...
    public void deletePatient(){
        String patID = InputValidator.readValidString("Patient ID (Enter 0 to cancel)",null, "delete", id -> !id.isEmpty() && patMan.findByID(id) != null, "Error: Patient ID does not exist!!");
        if (patID == null){
    ...
    public void updatePat(){
        String patID = InputValidator.readNonEmptyString("Patient ID (Enter 0 to cancel to abort update)");
        if(patID.equals("0")) {
        ...
        Patient updPat = new Patient(patID, newName, newSex, newAddr, newPhone, newDOB, newDiag, newDoc, newStatus, oldPat.getCreateDate(), new Date());
